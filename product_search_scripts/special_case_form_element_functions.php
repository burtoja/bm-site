 ...
 */
function add_cooling_towers_special_elements($product_category, $unique_id, $selectedValues = []) {
    $form_element = '';
    $form_element .= build_pulldown_menu($product_category, $unique_id, "media", $selectedValues['media'] ?? '');
    $form_element .= build_pulldown_menu($product_category, $unique_id, "material", $selectedValues['material'] ?? '');
    return $form_element;	
...
function add_switchgear_special_type_element($product_category, $unique_id, $selectedValues = []) {
    $form_element = '';
    $form_element .= build_pulldown_menu($product_category, $unique_id, "other_filters", $selectedValues['other_filters'] ?? '');
    return $form_element;	
}
 ...
 */
function add_generators_special_elements($product_category, $unique_id, $selectedValues = []) {
    $form_element = '';
    $form_element .= build_pulldown_menu($product_category, $unique_id, "phase", $selectedValues['phase'] ?? '');
    $form_element .= build_pulldown_menu($product_category, $unique_id, "voltage_single_phase", $selectedValues['voltage_single_phase'] ?? '');
    $form_element .= build_pulldown_menu($product_category, $unique_id, "voltage_three_phase", $selectedValues['voltage_three_phase'] ?? '');
    $form_element .= build_pulldown_menu($product_category, $unique_id, "power_max", $selectedValues['power_max'] ?? '');
    $form_element .= build_pulldown_menu($product_category, $unique_id, "power_rated", $selectedValues['power_rated'] ?? '');
    $form_element .= build_pulldown_menu($product_category, $unique_id, "use", $selectedValues['use'] ?? '');
    return $form_element;
}

/**
 * Builds storage tank special filter element pull-down menu
 */
function add_storage_tanks_special_elements($product_category, $unique_id, $selectedValues = []) {
    $form_element = '';
...
/**
 * Builds heat exchanger special filter element pull-down menu
 */
function add_heat_exchangers_special_elements($product_category, $unique_id, $selectedValues = []) {
    $form_element = '';
    $special_keys = ["flow", "material", "media"];
    foreach ($special_keys as $key) {
        $form_element .= build_pulldown_menu($product_category, $unique_id, $key, $selectedValues[$key] ?? '');
    }
    return $form_element;
...
/**
 * Builds diesel special filter element pull-down menu
 */
function add_diesel_engines_special_elements($product_category, $unique_id, $selectedValues = []) {
    $form_element = '';
    $special_keys = ["size", "cylinders", "fuel", "block", "assembly", "brand"];
    foreach ($special_keys as $key) {
        $form_element .= build_pulldown_menu($product_category, $unique_id, $key, $selectedValues[$key] ?? '');
    }
    return $form_element;
...
/**
 * Builds electric motor special filter element pull-down menu
 */
function add_electric_motors_special_elements($product_category, $unique_id, $selectedValues = []) {
    $form_element = '';
    $special_keys = ["voltage", "horsepower", "service_factor", "frequency", "phase", "current", "misc"];
    foreach ($special_keys as $key) {
        $form_element .= build_pulldown_menu($product_category, $unique_id, $key, $selectedValues[$key] ?? '');
    }
    return $form_element;
...
/**
 * Builds economizer special filter element pull-down menu
 */
function add_economizers_special_elements($product_category, $unique_id, $selectedValues = []) {
    $form_element = '';
    $special_keys = ["application", "classification", "material"];
    foreach ($special_keys as $key) {
        $form_element .= build_pulldown_menu($product_category, $unique_id, $key, $selectedValues[$key] ?? '');
    }
    return $form_element;
...
/**
 * Builds air compressor special filter element pull-down menu
 */
function add_air_compressors_special_elements($product_category, $unique_id, $selectedValues = []) {
    $form_element = '';
    $special_keys = ["power_source", "horsepower", "voltage", "lubrication", "pressure", "flow", "portability", "material"];
    foreach ($special_keys as $key) {
        $form_element .= build_pulldown_menu($product_category, $unique_id, $key, $selectedValues[$key] ?? '');
    }
    return $form_element;
...
/**
 * Builds deareators special filter element pull-down menu
 */
function add_deareators_special_elements($product_category, $unique_id, $selectedValues = []) {
    $form_element = '';
    $special_keys = ["material"];
    foreach ($special_keys as $key) {
        $form_element .= build_pulldown_menu($product_category, $unique_id, $key, $selectedValues[$key] ?? '');
    }
    return $form_element;
